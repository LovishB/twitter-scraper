...
export async function GET(request: Request) {
  try {
    ...
    // 5. Upsert results into raw_tweets
    ...
    if (settings?.filtering_prompt && tweets.length > 0) {
      console.log("Starting AI filtering with prompt:", settings.filtering_prompt.substring(0, 100) + "...");
      console.log("Sending", tweets.length, "tweets to AI for filtering");
      ...
      console.log("AI returned", aiResults.length, "selected tweets out of", tweets.length);
      aiResults.forEach((r) => {
        console.log(`  - Tweet ${r.tweet_id}: score=${r.relevance_score}, reply="${r.reply.substring(0, 80)}..."`);
      });

      // Look up raw_tweet UUIDs for the selected tweets
    ...
    // 7. Return result (no filtering prompt set)
